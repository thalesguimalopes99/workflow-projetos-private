...
Ele pertence ao repositório separado `~/aiox-core` (git próprio, remoto próprio), e
...
|---|---|---|---|
| **Fonte canônica** | `~/aiox-core/.aiox-core/development/agents/<id>.md` | persona completa, atual | ~25 KB |
...
- **Não reinstale o squad AIOX a partir desta pasta.** Instale do `~/aiox-core`, que é
...
```bash
C=~/aiox-core/.aiox-core/development/agents
...
```
~/aiox-core/.aiox-core/development/agents/<id>.md   # fonte
```

`~/aiox-core` é repo git com remoto próprio — o versionamento do AIOX acontece lá, não aqui.
