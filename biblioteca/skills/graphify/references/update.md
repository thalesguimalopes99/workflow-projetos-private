...
**Curation record on a re-cluster.** Skipping Steps 5–9 also skips Step 5.5, so handle its two artifacts by hand here — the intermediates are gone, but `graph.json` persists and carries everything needed:

- `GRAPH_GAPS.md` — the corpus did not change, so every corpus-level gap still holds. **Leave the file alone**, except the one item that depends on centrality: AMBIGUOUS edges touching a god node. New communities mean new god nodes, so recheck that item against the refreshed `GRAPH_REPORT.md` and edit only that bullet.
- `INGEST_LOG.md` — **append an entry anyway.** Relabelling communities is a judgment call, and one that silently rewrites how every future traversal reads. Record the run as `cluster-only`, the old labels, the new labels, and what drove the change.
