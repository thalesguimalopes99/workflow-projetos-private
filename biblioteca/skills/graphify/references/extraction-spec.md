...
ATTRIBUTE, NOT NODE - do not create a node for something that is a *predicate of* another
  thing rather than a thing in its own right. A metric, score, benchmark result, price,
  version number, date, or measurement is an assertion ABOUT some entity: store it as an
  attribute on that entity's node, never as its own node. A benchmark earns a node only
  when a source discusses the benchmark ITSELF (its methodology, its construction, its
  critique) - not when a source merely reports a score on it. The same holds for a price
  (attribute of the product), a version (attribute of the software), and a result
  (attribute of whatever was evaluated). Creating these speculatively produces degree-1
  leaf nodes that inflate the graph and carry no traversable relationship. This is the
  same principle as the `rationale` rule above, generalized: ask "is this a thing, or is
  this something said about a thing?" - only the former becomes a node.

SOURCE RELIABILITY - reliability of a SOURCE is a different axis from confidence in an
  EDGE. An edge can be EXTRACTED with confidence_score 1.0 (the source states it
  plainly) while the source itself is promotional, self-interested, or methodologically
  thin. Never let one stand in for the other. On every node whose content comes from a
  document, paper, image, or clipped web page, set:
    "source_reliability": one of "peer_reviewed" | "primary_documentation" |
       "independent_reporting" | "self_reported" | "interested_party" | "unknown"
    "source_caveats": array of short strings naming concrete limitations you can point
       to in the text. Only list what you actually observed - never boilerplate.
       Examples: "vendor announcing its own product", "no absolute numbers, only
       relative claims", "competitors unnamed", "methodology stated for one evaluation
       only", "no sample size", "undated".
  Use "interested_party" whenever the source stands to gain from the claim (a launch
  post, a sales page, a case study written by the vendor). Leave "source_caveats" as an
  empty array when you genuinely found none. For code nodes, omit both fields.
  Provenance fields (source_url, author, captured_at) say WHERE it came from; these two
  say HOW MUCH IT SHOULD BE TRUSTED. Both are needed.
...
{"nodes":[{"id":"auth_session_validatetoken","label":"Human Readable Name","file_type":"code|document|paper|image|rationale|concept","source_file":"<FILE_LIST path verbatim>","source_location":null,"source_url":null,"captured_at":null,"author":null,"contributor":null,"source_reliability":null,"source_caveats":[]}],"edges":[{"source":"node_id","target":"node_id","relation":"calls|implements|references|cites|conceptually_related_to|shares_data_with|semantically_similar_to|rationale_for","confidence":"EXTRACTED|INFERRED|AMBIGUOUS","confidence_score":1.0,"source_file":"<FILE_LIST path verbatim>","source_location":null,"weight":1.0}],"hyperedges":[{"id":"snake_case_id","label":"Human Readable Label","nodes":["node_id1","node_id2","node_id3"],"relation":"participate_in|implement|form","confidence":"EXTRACTED|INFERRED","confidence_score":0.75,"source_file":"<FILE_LIST path verbatim>"}],"input_tokens":0,"output_tokens":0}
