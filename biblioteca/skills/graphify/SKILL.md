...
### Step 5.5 - Curation record (always)

Two artifacts that answer questions the graph itself cannot: **what the corpus does not
cover**, and **why this run made the calls it made**. Both run on every build. Neither
touches the graph.

First collect the mechanical signals:
...
# Files detected but that produced no node — dropped chunks, failed extraction, empty files.
detected = {f for fl in (detect.get('all_files') or detect['files']).values() for f in fl}
covered  = {n.get('source_file') for n in extract.get('nodes', []) if n.get('source_file')}
covered |= {Path(p).name for p in covered if p}
silent   = sorted(f for f in detected if f not in covered and Path(f).name not in covered)

amb = [e for e in extract.get('edges', []) if e.get('confidence') == 'AMBIGUOUS']
flagged = [n for n in extract.get('nodes', []) if n.get('source_caveats')]
low = [n for n in extract.get('nodes', []) if n.get('source_reliability') in ('interested_party','self_reported')]

print('SILENT FILES (detected, produced no node):', len(silent))
for f in silent[:25]: print('  -', f)
print('AMBIGUOUS EDGES:', len(amb))
for e in amb[:15]: print('  -', e.get('source'), '->', e.get('target'), '|', e.get('relation'))
print('NODES WITH SOURCE CAVEATS:', len(flagged))
for n in flagged[:15]: print('  -', n.get('label'), '|', n.get('source_reliability'), '|', '; '.join(n.get('source_caveats', [])))
print('NODES FROM LOW-RELIABILITY SOURCES:', len(low))
print('SKIPPED SENSITIVE:', detect.get('skipped_sensitive', []))
"
```

**A. Write `graphify-out/GRAPH_GAPS.md`** — overwrite it whole on every build, so it always
describes the current corpus. Keep it out of `GRAPH_REPORT.md`: `generate()` rewrites that
file on every build *and* on `cluster-only`, which would silently eat an appended section.
State what the graph *cannot* answer and what would fix it. Draw on the signals above plus
what you saw while extracting:

- Files detected that produced no node — name them and say why (unreadable, empty, chunk failed).
- Assets referenced in the corpus but absent from it — figures, images, linked PDFs that carry
  substance no text restates. Say what would change if they were added.
- Claims resting on a single low-reliability source — list the node and the caveat.
- AMBIGUOUS edges touching a god node — an uncertain link at a hub distorts every traversal
  through it.
- Anything the corpus asserts without attribution, where the assertion is load-bearing.

Write each gap so it is actionable: the missing thing, why it matters, what to do. When the
corpus genuinely has no gap worth naming, write the file with a single line saying so — never
pad it with invented ones.

**B. Append one entry to `graphify-out/INGEST_LOG.md`** (create if absent; **append-only —
never rewrite or trim prior entries**). `cost.json` records what a run *cost* and the
manifest records what it *touched*; neither records what you *decided*. Six months on,
that is the difference between a deliberate call and an accident. One entry per build:

```markdown
## [YYYY-MM-DD] <full | update | cluster-only> — <corpus name>

- **Corpus:** N files (X code, Y docs, Z papers) · **Graph:** N nodes, N edges, N communities
- **Communities named:** the labels you chose, and what signal drove each
- **Judgment calls:** what became a node vs. an attribute, and why. Especially any metric,
  benchmark, price, or result you deliberately kept as an attribute — name it, so a later
  run does not silently reverse the decision.
- **Sources flagged:** which sources got a non-neutral `source_reliability`, and on what basis
- **Left out:** what you saw and chose not to extract, with the reason
- **Open:** what this run could not settle
```

Use the real date (`date -u +%Y-%m-%d`). Keep each entry short — decisions and their reasons,
not a summary of the corpus. Nothing here feeds the graph; it is the audit trail for the
human, and it is the only artifact that survives a `--force` rebuild with the reasoning intact.
...
Graph complete. Outputs in PATH_TO_DIR/graphify-out/
  ...
  GRAPH_GAPS.md         - what the corpus does NOT cover, and what would fix it
  INGEST_LOG.md         - append-only record of this run's judgment calls
